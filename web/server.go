...
func (s *Server) handleBlacklist(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.lm.AddToBlacklist(name); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	if originals, err := s.db.GetOriginalsForDisplay(name); err == nil {
		for _, o := range originals {
			_ = s.lm.AddToBlacklist(o)
		}
	}
	writeJSON(w, map[string]string{"status": "ok"})
}

func (s *Server) handleBlacklistExport(w http.ResponseWriter, r *http.Request) {
	names := s.lm.ExportBlacklist()
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Content-Disposition", "attachment; filename=blacklist_export.json")
	json.NewEncoder(w).Encode(names)
}

func (s *Server) handleBlacklistImport(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if err := s.lm.ImportBlacklist(names); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleUnblacklist(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.lm.RemoveFromBlacklist(name); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	if originals, err := s.db.GetOriginalsForDisplay(name); err == nil {
		for _, o := range originals {
			_ = s.lm.RemoveFromBlacklist(o)
		}
	}
	writeJSON(w, map[string]string{"status": "ok"})
}
...
func (s *Server) handleWhitelist(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.lm.AddToWhitelist(name); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleUnwhitelist(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.lm.RemoveFromWhitelist(name); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleRename(w http.ResponseWriter, r *http.Request) {
	...
	if from == "" || to == "" {
		http.Error(w, "from/to empty", http.StatusBadRequest)
		return
	}
	if err := s.db.RenameSmart(from, to); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleFinished(w http.ResponseWriter, r *http.Request) {
	...
	if body.Done == nil {
		if finished, err := s.db.IsFinished(name); err == nil {
			if finished {
				_ = s.db.DeleteFinished(name)
			} else {
				_ = s.db.InsertFinished(name)
			}
		} else {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		...
		if err := s.db.InsertFinished(name); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	} else {
		if err := s.db.DeleteFinished(name); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (s *Server) handleCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, r1 := range rows {
		...
		if strings.TrimSpace(r1.NewCSV) != "" {
			nlist = strings.Split(r1.NewCSV, "||")
		}
		if strings.TrimSpace(r1.FinishedCSV) != "" {
			flist = strings.Split(r1.FinishedCSV, "||")
		}
...
func (s *Server) handleSetFirstLaunchDate(w http.ResponseWriter, r *http.Request) {
	...
	if name == "" || date == "" {
		http.Error(w, "name/date empty", http.StatusBadRequest)
		return
	}
	...
	if err := s.db.UpsertFirstLaunchOverride(name, date); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleSetFinishedDate(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.db.UpsertFinishedAt(name, date); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleHistoryDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.db.DeleteActivity(p, st, et); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]string{"status": "ok"})
...
func (s *Server) handleDayTimeline(w http.ResponseWriter, r *http.Request) {
	...
	for _, row := range rows {
		...
		if es > ss {
			segs = append(segs, Seg{Name: row.Name, StartSec: ss, EndSec: es})
		}
	}
...
func (s *Server) handleImport(w http.ResponseWriter, r *http.Request) {
	...
	for _, a := range payload.Activities {
		...
		if mode == "merge" {
			var exists bool
			if err := tx.Get(&exists, `SELECT EXISTS(SELECT 1 FROM activities WHERE process_name=? AND start_time=? AND end_time=?)`, pname, stUTC, etUTC); err != nil {
				rollback()
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
		...
		if _, err := tx.Exec(`INSERT INTO activities (process_name, start_time, end_time, duration, date, first_launch) VALUES (?,?,?,?,?,?)`, pname, stUTC, etUTC, a.Duration, dateStr, a.FirstLaunch); err != nil {
			rollback()
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, name := range payload.Whitelist {
		...
		if _, err := tx.Exec(`INSERT OR IGNORE INTO whitelist (name) VALUES (?)`, n); err != nil {
			rollback()
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, name := range payload.Blacklist {
		...
		if _, err := tx.Exec(`INSERT OR IGNORE INTO blacklist (name) VALUES (?)`, n); err != nil {
			rollback()
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, r1 := range payload.RenameMap {
		...
		if _, err := tx.Exec(`INSERT INTO rename_map (original_name, display_name) VALUES (?, ?) ON CONFLICT(original_name) DO UPDATE SET display_name=excluded.display_name`, orig, disp); err != nil {
			rollback()
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, f := range payload.FinishedGames {
		...
		if _, err := time.Parse("2006-01-02", strings.TrimSpace(f.FinishedAt)); err != nil {
			continue
		}
		if _, err := tx.Exec(`INSERT INTO finished_games (name, finished_at) VALUES (?, ?) ON CONFLICT(name) DO UPDATE SET finished_at=excluded.finished_at`, name, f.FinishedAt); err != nil {
			rollback()
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, fl := range payload.FirstLaunchOverrides {
		...
		if _, err := time.Parse("2006-01-02", strings.TrimSpace(fl.FirstDate)); err != nil {
			continue
		}
		if _, err := tx.Exec(`INSERT INTO first_launch_override (name, first_date) VALUES (?, ?) ON CONFLICT(name) DO UPDATE SET first_date=excluded.first_date`, name, fl.FirstDate); err != nil {
			rollback()
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
